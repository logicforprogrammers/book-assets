...
% "caf": helper rule commit_author_file
% true iff author A changed file F as part of commit C
caf(C, A, F) :-
...
high_churn(File) :-
    ...
    A1 @< A2, A2 @< A3. % `@<` compares atoms by name. 
                        % this ensures all authors are different
...
% helper rule to use for the `setof` Goal in `file_problems`
